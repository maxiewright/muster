...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="dark min-h-dvh bg-slate-50 text-slate-900 antialiased dark:bg-zinc-950 dark:text-zinc-100">
        <div class="relative isolate flex min-h-dvh items-center justify-center overflow-hidden px-6 py-10">
            <div class="absolute inset-0 -z-20 bg-[radial-gradient(circle_at_15%_20%,rgba(16,185,129,0.10),transparent_30%),radial-gradient(circle_at_80%_0%,rgba(56,189,248,0.08),transparent_35%),linear-gradient(180deg,#f8fafc_0%,#f1f5f9_50%,#f8fafc_100%)] dark:bg-[radial-gradient(circle_at_15%_20%,rgba(16,185,129,0.28),transparent_30%),radial-gradient(circle_at_80%_0%,rgba(245,158,11,0.2),transparent_35%),linear-gradient(180deg,#09090b_0%,#18181b_50%,#09090b_100%)]"></div>
            ...
            <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-xl backdrop-blur dark:border-zinc-600/90 dark:bg-zinc-900/95 dark:shadow-2xl">
                ...
                <form method="POST" action="{{ route('setup.store') }}" class="mt-6 space-y-4">
                    ...
                    <flux:field>
                        <flux:label class="!text-slate-700 dark:!text-zinc-200">Password</flux:label>
                        <flux:input name="password" type="password" required autocomplete="new-password" viewable />
                        <flux:error name="password" />
                    </flux:field>

                    <flux:field>
                        <flux:label class="!text-slate-700 dark:!text-zinc-200">Confirm Password</flux:label>
